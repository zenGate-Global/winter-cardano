 ...
 * Or apply a shape and convert the provided data struct to Cbor encoded data.
 */
function to<T = Data>(data: Exact<T>, type?: T, recType?: string): Datum | Redeemer {
  ...
  switch (shapeType) {
    ...
    case 'enum': {
      ...
      if (isBoolean(shape)) {
        ...
        if (struct === null) return new Constr(1, []);
          ...
          return new Constr(0, [castTo<T>(struct, fields[0], recType, recShape)]);
        }
      }
      switch (typeof struct) {
        case 'string': {
          ...
          const enumIndex = (shape as TEnum).anyOf.findIndex(
            (s: TLiteral) =>
              s.dataType === 'constructor' && s.fields.length === 0 && s.title === struct
          );
    ...
    case 'list': {
      ...
      if (shape.items instanceof Array) {
        ...
        return struct.map((item) => castTo<T>(item, shape.items, recType, recShape));
      }
    }
    case 'map': {
