  ...
  const address =
    options.addressType === 'Base'
      ? C.BaseAddress.new(
          ...
          .to_address()
          .to_bech32(undefined)
      : C.EnterpriseAddress.new(networkId, C.StakeCredential.from_keyhash(paymentKeyHash))
          .to_address()
          .to_bech32(undefined);

  const rewardAddress =
    options.addressType === 'Base'
      ? C.RewardAddress.new(networkId, C.StakeCredential.from_keyhash(stakeKeyHash))
          .to_address()
          .to_bech32(undefined)
      : null;
