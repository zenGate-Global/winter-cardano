...
import {
  type ObjectDatum,
  type ObjectDatumFields,
  type ObjectDatumParameters,
} from "../src";
...
describe("Creating an EventFactory", async () => {
  ...
  const previewTxHash =
    "88a5d805c7e4579d89ae1792b79660716318ef52c1d0e89b8529f81db279c12c";
  const pVersion = 1;
  ...
  console.log("sharedEvents: ", sharedEvents);
  ...
  const testPolicyId =
    "de51e5038d491a98698045a464f4b7237fe181aa8cac8d68da1986f2";

  let datum: ObjectDatum;
  ...
  it("Should sort provided utxos by output reference", async () => {
    const utxos = await eventFactory.getWalletUtxos();
    const refs = await eventFactory.getUtxosByOutRef(
      utxos.map((utxo) => utxo.input)
    );
  ...
  it("Should get an ObjectDatum from parameters", () => {
    datum = EventFactory.getObjectDatumFromParams(objectDatumParams);
  ...
  it("Should build a singleton event tx", async () => {
    ...
    const unsignedTx = await eventFactory.mintSingleton(
      tokenName,
      utxos,
      datum
    );
